...
const tarefa = (id, novaTarefa) => `<div class="list-items">
<div class="span-area">
<input type="checkbox" onchange="marcarTarefa(${id})" class="checkbox-round"/>
<span id='${id}'>${novaTarefa}</span>
</div>
<div>
...
const marcarTarefa = (id) => {
  ...
  if (strike) {
    ...
    document.getElementById(
      id
    ).innerHTML = `<strike id='strike${id}'>${tarefaConcluida}</strike`;
  }
};
...
function exibirLista() {
  ...
  if (tarefas) {
    tarefas.forEach((tarefaListada) => {
      id++;
      document.querySelector("#lista-tarefas").innerHTML += tarefa(
        id,
        tarefaListada
      );
    });
...
const removerTarefa = (id) => {
  ...
  const listaTarefas = JSON.parse(localStorage.getItem("lista-tarefas"));
  const novaListaTarefa = listaTarefas.filter(
    (tarefa) => tarefa !== tarefaDeletada
  );
